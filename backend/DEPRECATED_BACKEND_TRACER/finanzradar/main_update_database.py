...
if __name__ == "__main__":
    ...
    tickers = list(json.loads(Path("finanzradar/featurestore/10_bronze/tickerSearchUS.json").read_text()).keys())
    
    # Fetch & write all data
    for source in data_sources:
        start_time = time.time()
        fetch_all(tickers=tickers, fetch_fn=source["fetch_fn"], output_dir=source["output_dir"], max_workers=source["max_workers"], executor=source["executor"], overwrite=source["overwrite"], filing_type=source["filing_type"], partition_cols=source["partition_cols"], key=source["key"])
