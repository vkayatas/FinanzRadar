<template>
  ...
  <!-- Loading / Error Handling -->
    <div v-if="store.stockInfo.loading" class="text-gray-500 dark:text-gray-400">
      Loading stock info...
    </div>
    <div v-else-if="store.stockInfo.error" class="text-red-500 dark:text-red-400">
      Error: {{ store.stockInfo.error }}
    </div>
    <div v-else-if="store.fundamentals.data">
      <!-- Header with logo and name TODO: Use clearbit for logo like i did with stock-vue, better load them locally: logo_url = f'https://logo.clearbit.com/{domain}' -->
      ...
      <p class="text-sm sm:text-base text-gray-500 dark:text-gray-400 mb-6">
          {{ $t('stocks.labels.lastUpdated') }}: {{ store.stockInfo.details.lastUpdated}}
